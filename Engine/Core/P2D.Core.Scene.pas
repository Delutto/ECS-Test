...
type
  { =========================================================================
    ...
    Never override Load/Start/Finish/Unload directly; those handle logging and call the corresponding Do* method.
  ========================================================================= }
   TScene2D = class
   private
      FWorld: TWorld;
...
procedure TScene2D.Start;
begin
   FActive := True;
   FPaused := False;
   FAccumulator := 0;
   {$IFDEF DEBUG}
   Logger.Info('[Scene] Starting: ' + FName);
   {$ENDIF}
   DoEnter;
end;

procedure TScene2D.Finish;
begin
   FActive := False;
   {$IFDEF DEBUG}
   Logger.Info('[Scene] Finishing: ' + FName);
   {$ENDIF}
   DoExit;
end;
