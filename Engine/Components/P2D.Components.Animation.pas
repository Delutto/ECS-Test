...
begin Result := Length(Frames); end;
...
procedure TAnimationComponent.Play(const AName: string);
begin
  if FCurrentName = AName then Exit;
  if FAnimations.IndexOf(AName) < 0 then Exit;
...
begin
  ...
  if not Assigned(FCurrent) or (FCurrent.FrameCount = 0) then Exit;

  ARect := FCurrent.Frames[FFrameIndex].SourceRect;

  if FFinished and not FCurrent.Loop then Exit;
  ...
  begin
    ...
    begin
      if FCurrent.Loop then FFrameIndex := 0
      else begin FFrameIndex := FCurrent.FrameCount - 1; FFinished := True; Break; end;
    end;
  end;
end;
