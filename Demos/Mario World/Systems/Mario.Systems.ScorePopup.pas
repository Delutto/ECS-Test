...
begin
   ...
   TW.Start('rise', @Tr.Position.Y, AWorldY, AWorldY - POPUP_RISE, POPUP_DURATION, @EaseOutQuad, {Loop=}False, {PingPong=}False, {OnComplete=}nil);
   E.AddComponent(TW);
end;
...
begin
   ...
   SpawnPopup(Ev.WorldX, Ev.WorldY, '+' + IntToStr(Ev.ScoreGained), ColorCreate(255, 240, 50, 255));   // bright yellow
end;

procedure TScorePopupSystem.OnCoinCollected(AEvent: TEvent2D);
begin
   SpawnPopup(TCoinCollectedEvent(AEvent).WorldX, TCoinCollectedEvent(AEvent).WorldY, 'COIN!', ColorCreate(255, 215, 0, 255)); // gold
end;
