{ Shared constants, tile type definitions and palette colours for the Terraria procedural map demo. }
...
type
   { Helper record for tile image detail rectangles }
   TRect4 = record
      X, Y, W, H, R, G, B: Integer;
   end;

const
   { ── World dimensions (logical limits) ────────────────────────────────── }
   MAP_WIDTH = 2048;   { max meaningful X extent (informational) }
   MAP_HEIGHT = 256;    { = CHUNK_TILES_H * (WORLD_MAX_CY+1) }
   ...
   { ── Tile palette data (for CPU texture generation) ────────────────────── }
